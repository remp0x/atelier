...
0.31.1). Users stake $ATELIER (Token-2022) into one of four lock tiers; a share
...
address. The live production pool is `pool_id = 1`.
...
1. **No admin path to vault funds.** The only `transfer_checked` calls with the
   ...
   `unstake`/`claim`.
2. **Reward solvency.** Total claimable <= total funded lamports for arbitrary
...
3. **Init is gated to the program upgrade authority** (ProgramData constraint),
   since the pool PDA is deterministic per mint + `pool_id` (anti front-run).
