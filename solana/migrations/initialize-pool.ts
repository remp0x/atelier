 ...
 * default to 15d 1x / 30d 4x / 60d 10x / 180d 20x; override with TIERS_JSON (an
 * array of { durationSecs, multiplierBps }). POOL_ID (default 1) selects the
 * pool instance for the staked mint (part of the pool PDA seeds). REWARD_DURATION_SECS (the linear-drip
...
const DEFAULT_TIERS = [
  { durationSecs: 15 * DAY, multiplierBps: 10_000 },
  { durationSecs: 30 * DAY, multiplierBps: 40_000 },
  { durationSecs: 60 * DAY, multiplierBps: 100_000 },
  { durationSecs: 180 * DAY, multiplierBps: 200_000 },
];
...
async function main(): Promise<void> {
  ...
  if (!Number.isInteger(poolId) || poolId < 0 || poolId > 255) {
    throw new Error("POOL_ID must be an integer 0-255");
  }
  ...
  const [pool] = PublicKey.findProgramAddressSync(
    [Buffer.from("pool"), stakedMint.toBuffer(), Buffer.from([poolId])],
    program.programId,
  );
  ...
  console.log("pool:           ", pool.toBase58(), `(pool_id ${poolId})`);
  ...
  const sig = await program.methods
    .initializePool(poolId, tiers as never, new BN(rewardDurationSecs), funder)
    .accountsPartial({
      admin,
