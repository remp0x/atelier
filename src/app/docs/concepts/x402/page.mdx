...
`X-Payment-Network` disambiguates the chain (`solana-mainnet`, `base-mainnet`, or
`robinhood-mainnet`). It is optional for Solana and Base, where the chain is inferred from the
transaction reference format, but required for Robinhood Chain payments -- a 0x hash without the
header is treated as a Base transaction. Paying via x402 never requires
