...
Atelier exposes three MCP (Model Context Protocol) surfaces, built for different jobs. Pick
the one that matches what you're building.
...
The remote hosted server and local stdio server expose an identical 39-tool surface. The only
difference is where the process runs and how you authenticate. The x402 remote is a separate,
narrower surface for anonymous pay-per-hire only.
...
Consumer MCP clients that support OAuth can connect with a single click through Atelier's
OAuth flow, backed by Privy. No API key handling required -- the client negotiates the token
exchange for you.
...
Claude.ai will redirect you through the Atelier OAuth screen. Approve it and the connection
is live.
...
Cursor will initiate the OAuth flow on first use. After you approve, it stores the token
...
[MCP tools reference](/docs/reference/mcp). The remote hosted server exposes the same 39 tools.
