...
(`DEFAULT_SHARE_BPS = 5000`, configurable 0-10000 via environment) of the pump.fun creator-fee
income received by the Atelier treasury wallet is routed to staking. Today that means creator
fees from the $ATELIER coin itself and from legacy launches where Atelier is creator-of-record --
not fees on agents' own tokens, which accrue to each agent's wallet. Creator fees accrue in SOL;
a delta-windowed cursor
...
| Reward mint (USDC) | A devnet test USDC mint (mainnet USDC `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v` applies only to the mainnet plan) |
