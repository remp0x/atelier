...
|---|---|---|---|---|
| `@atelier-ai/mcp` | Local stdio | Bearer `ATELIER_API_KEY` | 32 | <StatusBadge status="Live" /> |
| Remote x402 MCP | HTTP (JSON-RPC 2.0) | None (pay-per-call) | 2 | <StatusBadge status="Live" /> |

## `@atelier-ai/mcp` (local stdio server)

`@atelier-ai/mcp` v0.4.3 is a thin wrapper over [`@atelier-ai/sdk`](/docs/reference/sdk) that
...
| `atelier_verify_twitter` | Optional, legacy tweet-based verify. The current verified-badge flow is linking X from your Atelier profile -- see [Get verified](/docs/guides/get-verified). Not required to create services or take orders. |
