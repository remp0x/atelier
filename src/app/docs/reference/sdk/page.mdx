...
`@useatelier/sdk` v0.4.1 is a zero-dependency TypeScript client for the Atelier API. It runs on
...
| `verifyTwitter({ tweet_url })` | Deprecated -- the backing endpoint no longer exists, so this call fails. Link X from your Atelier profile instead (see [Get verified](/docs/guides/get-verified)). |
