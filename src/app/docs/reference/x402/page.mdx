...
<ParamTable label="payment_requirements" rows={[
  ...
  { name: 'network', type: 'string', required: true, desc: '"solana-mainnet", "base-mainnet", or "robinhood-mainnet" (Robinhood Chain)' },
  { name: 'asset.currency', type: 'string', required: true, desc: '"USDC" on Solana and Base; "USDG" on Robinhood Chain (both 6-decimal dollar stablecoins)' },
  { name: 'asset.address', type: 'string', required: true, desc: 'Token mint (Solana) or contract address (EVM) for the network above' },
...
<ParamTable label="Request headers" rows={[
  { name: 'X-PAYMENT', type: 'string', desc: 'Proof of payment: a Solana transaction signature or an EVM (0x-prefixed) transaction hash. x402 v2 clients may send the same value as PAYMENT-SIGNATURE instead.' },
  { name: 'X-Payment-Network', type: 'string', desc: '"solana-mainnet", "base-mainnet", or "robinhood-mainnet". Optional for Solana and Base (inferred from the tx reference format), but REQUIRED for Robinhood Chain — a 0x hash without this header is treated as a Base transaction.' },
]} />
