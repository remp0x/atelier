...
import { WalletSelector } from '@/components/atelier/stake/WalletSelector';
import type { StakeWalletMode } from '@/components/atelier/stake/useStakeTxSender';
...
export function StakePageClient() {
  const { ready, login, solanaAddress } = useAtelierAuth();
  const { publicKey: externalPubkey, connected: externalConnected } = useWallet();

  const [walletMode, setWalletMode] = useState<StakeWalletMode>('embedded');
  const activeAddress =
    walletMode === 'external'
      ? externalConnected && externalPubkey
        ? externalPubkey.toBase58()
        : null
      : solanaAddress;
  ...
  const [positions, setPositions] = useState<(StakePositionAccount | null)[]>([null, null, null, null]);
  ...
  }, [activeAddress, fetchChainData, fetchAtelierBalance]);
  ...
  }, [fetchStats, fetchChainData, fetchAtelierBalance, activeAddress]);
      ...
      <div className="px-4 py-6 md:px-8 md:py-8 border-b border-gray-100 dark:border-neutral-900">
        ...
        >
          ...
          <p className="font-mono text-[12px] text-gray-500 dark:text-neutral-500 max-w-xl leading-relaxed">
            Lock $ATELIER to earn a share of Atelier&apos;s platform revenue, paid in SOL. Longer
            locks earn higher multipliers.
          </p>
        </motion.div>
      </div>
                ...
                {activeAddress ? (
                  ...
                  >
                    <p className="font-mono text-[12px] text-gray-500 dark:text-neutral-500">
                      Sign in to stake $ATELIER and earn SOL rewards.
                    </p>
                    <button
                      type="button"
                      onClick={login}
                  ...
                  <div className="rounded-xl border border-gray-200 dark:border-neutral-800 bg-white dark:bg-[#0d0d0d] p-6 text-center">
                    <p className="font-mono text-[12px] text-gray-500 dark:text-neutral-500">
                      Connect an external wallet above to stake from it.
                    </p>
                  </div>
                )}
              </div>
