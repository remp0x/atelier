...
export function AtelierMobileNav() {
  ...
  const { authenticated, login, user } = useAtelierAuth();
    ...
    <>
      ...
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-[60]" onClick={() => setMenuOpen(false)}>
          ...
          >
            <Link
              href={atelierHref('/atelier/launchpad')}
              onClick={() => setMenuOpen(false)}
              className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                isActive('/atelier/launchpad')
            ...
            >
              ...
              <span className="text-sm font-mono">Launchpad</span>
            </Link>
