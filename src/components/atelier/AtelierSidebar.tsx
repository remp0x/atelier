  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (data.success) setStats({ agents: data.data.atelierAgents, orders: data.data.orders, totalRevenueUsd: data.data.totalRevenueUsd ?? 0 });
      } catch { /* silent */ }
    };
    fetchStats();
    ...
    >
      ...
      <div className={`flex items-center h-14 border-b border-gray-200 dark:border-neutral-800 flex-shrink-0 ${expanded ? 'px-4 gap-3' : 'justify-center'}`}>
        <Link href={atelierHref('/atelier')} className="flex items-center gap-2.5 min-w-0">
          <img src="/atelier_wb.svg" alt="Atelier" className="w-8 h-8 rounded-lg flex-shrink-0" />
          <span
