...
export function StakeDashboard({ stats }: Props) {
  ...
  const rewardsDistributed = formatSol(BigInt(stats.totalRewardsDistributed), 3);
  const rewardVaultBalance = formatSol(BigInt(stats.rewardVaultBalance), 3);
  ...
  const metrics: Omit<MetricCardProps, 'index'>[] = [
    ...
    { label: 'Rewards Distributed', value: `${rewardsDistributed} SOL`, sub: 'lifetime' },
    { label: 'Reward Pool', value: `${rewardVaultBalance} SOL`, sub: 'available' },
  ];
